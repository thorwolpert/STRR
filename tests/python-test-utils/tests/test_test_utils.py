...
def test_pure_logic_skips_docker(request):
    ...
    postgres_fixture = fixture_manager.getfixturedefs("postgres_container", request.node)
    redis_fixture = fixture_manager.getfixturedefs("redis_container", request.node)

    postgres_started = any(getattr(d, "cached_result", None) is not None for d in postgres_fixture) if postgres_fixture else False
    redis_started = any(getattr(d, "cached_result", None) is not None for d in redis_fixture) if redis_fixture else False
