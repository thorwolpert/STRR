    ...
    test('Complete Application Flow', async ({ page }) => {
      ...
      await completeStep1(
        ...
        async () => {
          ...
          await expect(getPropertyRequirementsList(page).getByRole('button', { name: 'Business Licence' })).not.toBeVisible() // business license should not be visible
        }
      )
      ...
      await completeStep3(
        ...
        async () => {
          ...
          await expect(requiredDocsList).not.toContainText('Local government short-term rental business licence')
        }
      )
